---
mode: agent
description: Remediate WCAG 2.1/2.2 Level AA violations found by a11y-scan/a11y-review — semantics, names/roles/state, keyboard, focus, contrast — with the minimal correct change, preferring native elements over ARIA. WCAG-only; verifies the fix.
---

Read and execute the skill at [`.developer/skills/accessibility/a11y-fix/SKILL.md`](../../.developer/skills/accessibility/a11y-fix/SKILL.md). Follow its Role, load the listed context,
and complete its TodoWrite workflow in order. Use the user's request as the skill arguments.
